...
class PetStateMachine extends StateNotifier<PetState> {
  ...
  PetState _transition(PetState current, PetEvent event) {
    return switch (event) {
      ...
      UserTapped()          => current == PetState.sleeping ? PetState.idle : PetState.excited,
      // Overlay-managed events — state machine returns idle, animator handles it
      UserBeating()         => PetState.idle,
      SleepRequested()      => PetState.sleeping,
      WakeRequested()       => PetState.idle,
      TalkingTomToggle()    => PetState.waiting,
      StartWalking(goRight: _) => PetState.running,
    };
  }
